var Dashboard = {
    ...
    init: function () {
        ...
        // Account for the {stage} added to the url by the apigateway
    ...
    //     this.getTeamData();
    //     this.render();
    //     $("#team-name").text("Team: " + team).prop('href', `${d.stage}admin/dashboard/team/${team}/`)
    //     $("title").text("Dashboard - " + team);
    //     $("select[name=team]").val(team)
    // },
    setSurvey: function (stage, survey_id, team) {
        this.stage = stage;
        this.survey = survey_id;

        this.getSurveyData();
        this.render();
        name = team + " (" + survey_id + ") dashboard"
    ...
    getSurveyData: async function () {
        var d = this;
        d.data = {};
        console.log("Calling survey data from " + `${d.stage}` + "api/v1/survey/" + `${d.survey}` + "/stats")
        $.ajax({
            url: `/${d.stage}api/v1/survey/${d.survey}/stats`,
            async: false,
    ...
    //     d.data = {};
    //     $.ajax({
    //         url: `${d.stage}api/v1/stats/?team=${d.team}&dateStart=${d.dateStart}&dateEnd=${d.dateEnd}`,
    //         async: false,
    //         success: function (result) {
    //             d.data = result;
    //         },
